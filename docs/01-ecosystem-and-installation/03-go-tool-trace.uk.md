...
Два слова на цій сторінці зустрічаються раніше, ніж книга їх пояснює. **Горутина** (goroutine) — це функція, яка виконується конкурентно з рештою програми; Go runtime сам розподіляє горутини по потоках ОС. Тема [Конкурентність](../05-concurrency/01-goroutines.md) розглядає їх. **GC** — це збирач сміття Go, а *пауза GC* — це коротка зупинка, під час якої збирач виконує частину своєї роботи.
...
Найпростіший шлях — без змін у коді. Зверніть увагу: `-trace` приймає лише
**один пакет** за раз:

```bash
go test -trace=trace.out ./counter   # один пакет
```
...
Файл потрапляє в каталог, з якого ви запустили команду, а не в каталог
пакета.
