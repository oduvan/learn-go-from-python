...
| Директорія | Значення |
|---|---|
| `cmd/<name>/` | Кожна піддиректорія містить одну програму, яку можна запустити: пакет з іменем `main`, який Go збирає в один бінарний файл (`cmd/server`, `cmd/cli`). Стандартне розташування, коли репозиторій містить кілька виконуваних файлів. |
