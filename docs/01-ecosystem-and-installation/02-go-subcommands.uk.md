...
Компілює у бінарний файл у поточній директорії (або за шляхом, вказаним через `-o`). Не запускає. Бінарний файл містить у собі Go runtime, тож на цільовій машині не потрібно встановлювати інтерпретатор.

Але він **не** автоматично повністю статичний. **cgo** — це частина Go, яка дозволяє Go-коду викликати C-код, і вона типово увімкнена, коли наявний C-тулчейн. З увімкненим cgo імпорт `net` чи `os/user` лінкується із системною libc:
...
Ставте `CGO_ENABLED=0`, коли потрібен бінарник, що працює в контейнері `scratch` чи distroless. Додайте `-trimpath`, щоб абсолютні шляхи до ваших джерел не потрапляли в бінарник.
...
- `-tags <tag>` — увімкнути build-теги: мітки, які вмикають чи вимикають цілі файли в збиранні. [Типи файлів](04-go-file-types.md) показує, як файл долучається до тега.
