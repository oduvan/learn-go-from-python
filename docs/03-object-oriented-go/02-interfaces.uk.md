...
## Твердження задоволення на етапі компіляції

У неявного задоволення є одна ціна: у файлі самого типу нічого не каже,
що́ він має реалізовувати, тож помилка на кшталт тієї, з отримувачем за
вказівником вище, виринає у *місці виклику*, можливо, в іншому пакеті.
Виправлення — однорядкове оголошення, що стверджує це там, де живе тип:

```go
var _ Adder = (*Counter)(nil)
```

Читайте його справа наліво: `(*Counter)(nil)` — це nil-вказівник *відомого
типу*, він присвоюється `Adder`, а результат відкидається через `_`.
Нічого не виконується і нічого не виділяється — єдина робота цього рядка
— змусити компілятор перевірити набір методів. Дужки навколо `*Counter`
потрібні, бо `*Counter(nil)` розібралося б як `*(Counter(nil))`.

Помилитеся — і збірка зупиниться саме тут, на рядку, що заявляє намір, а
не там, де хтось уперше спробував використати цей тип:
...
Варто знати обидві форми. Для `Circle`, чий `Area` має отримувача за
значенням, компілюється будь-яка з них:

```go
var _ Shape = Circle{}         // ок
var _ Shape = (*Circle)(nil)   // також ок — до набору методів *Circle входить Area
```

Стверджуйте ту форму, яку насправді використовуватимуть виклики.
Застосовуйте це для типів, чиє єдине призначення — задовольняти
інтерфейс: це коштує один рядок і перетворює заплутану помилку десь-інде
на очевидну помилку прямо тут.
...
| твердження задоволення на етапі компіляції | `var _ Adder = (*Counter)(nil)` |
