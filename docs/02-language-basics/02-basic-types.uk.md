...
`int` — це **не** псевдонім `int64`. Навіть на 64-бітній машині, де вони мають однаковий розмір, це *різні типи* — дивіться [перетворення типів](03-type-conversions.md).
...
Для перебору по рунах використовуйте цикл `for ... range`. [Керування потоком виконання](05-control-flow.md) докладно розглядає цикли; тут кожен прохід дає байтовий індекс `i` та руну `r`:
...
Go має другий спосіб записати рядок. **Сирий рядковий літерал** пишеться між зворотними лапками. Усередині нього немає екранування: зворотна риска — це просто зворотна риска, і такий рядок може займати кілька рядків тексту:
...
Сирі рядки підходять для шляхів до файлів, регулярних виразів і багаторядкового тексту. Єдине, що сирий рядок не може містити, — це зворотну лапку.

> **З досвіду Python:** сирий рядок — це як `r"""..."""` у Python: без екранування, і він може займати кілька рядків.
