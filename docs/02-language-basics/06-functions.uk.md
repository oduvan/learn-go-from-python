...
У прикладах нижче використано три речі, які докладно розглядають
наступні кілька статей. Поки що цього достатньо, щоб їх прочитати:

- `type Reader struct { ... }` оголошує тип запису з іменованими
  полями. [Структури](10-structs.md) розглядають це.
- `*Reader` — це вказівник на `Reader`, а `&Reader{...}` створює
  `Reader` і повертає вказівник на нього. [Вказівники](08-pointers.md)
  розглядають обидва.
- `type Option func(*Server)` дає ім'я типу функції: `Option` — це
  будь-яка функція, яка приймає `*Server`. [Власні
  типи](09-custom-types.md) розглядають іменовані типи.
