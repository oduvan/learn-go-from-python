...
1. **Базове ім'я береться з власного оголошення `package` пакету**, а не зі шляху імпорту. Зазвичай вони збігаються, але не завжди. Шлях імпорту `math/rand/v2` містить пакет, який оголошує себе як `package rand`, тож ви пишете `rand.IntN`, а не `v2.IntN`.
...
Порожній імпорт компілює та лінкує пакет, але не прив'язує жодної назви у вашому файлі. Посилатися на що-небудь із нього не можна. Мета — запустити функцію `init()` пакету заради її побічних ефектів. (`init()` — це функція, яку Go викликає автоматично під час старту програми, до `main`; [Пакети та видимість](../04-packages-and-modules/01-packages-and-visibility.md) розглядає її.) Наприклад, так реєструє себе драйвер бази даних:
