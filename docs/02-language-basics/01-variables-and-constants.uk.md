...
`nil`-мапа — це єдина несподіванка тут: читання з неї дає нульове значення, а запис зупиняє програму з помилкою часу виконання. [Мапи](12-maps.md) показують, як створити мапу, у яку можна писати.
...
Go повідомляє про невдачу, повертаючи `error` як останнє значення, а не через виняток. `if err != nil` перевіряє це. [Помилки](07-errors.md) детально розглядають цей підхід.
...
`iota` — це зумовлений ідентифікатор, який скидається до 0 на початку кожного блоку `const` і зростає на 1 у кожному рядку блоку.
