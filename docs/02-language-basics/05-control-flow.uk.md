...
Умова **має бути типу `bool`** — немає «правдивих» та «хибних» значень. Дивіться [базові типи](02-basic-types.md).
...
`%q` друкує рядок у подвійних лапках, з екрануванням на кшталт `\n`, яке показане, а не застосоване. [Форматування через fmt](../06-text-time-and-data/02-formatting-with-fmt.md) охоплює всі дієслова формату.
...
case 1:
    fmt.Println("one")
    fallthrough        // безумовно провалюється в наступну гілку
case 2:
...
`fallthrough` виконує тіло наступної гілки, навіть хоча `x` не дорівнює 2.
...
(Це просто цикл `for`, написаний складним способом, — саме тому `goto` рідко потрібен.)
