...
Конкатенація з `+` у щільному циклі має складність O(n²) — кожен `+` копіює весь попередній префікс. `strings.Builder` має складність O(n): він збирає фрагменти і копіює їх один раз, коли ви викликаєте `String()`. (Цикл `for ... range` і зріз `words` розглянуті пізніше, у статтях [Керування потоком](05-control-flow.md) і [Масиви та зрізи](11-arrays-and-slices.md).)
...
Зрізи, мапи та структури мають власні статті в цій темі. Поки що просто запам'ятайте, на яких із них працює `==`.
