...
Оскільки порівнювана структура підтримує `==`, вона напряму годиться як
ключ — зручно для **складеного ключа** (розрідженої сітки, мемоізації за
кількома значеннями, усунення дублікатів за кортежем). Пошук порівнює
ключ поле за полем, тож однакові структури відображаються в той самий
запис.
...
Якщо структура має хоч одне непорівнюване поле (зріз, map чи функцію),
весь тип стає непорівнюваним, і саме оголошення map є помилкою
компіляції: `invalid map key type`.
...
`maps.Keys` повертає *ітератор*: значення, яке можна перебирати циклом
`range`, по одному ключу за раз, без попередньої побудови зрізу.
([Ітератори](../06-text-time-and-data/06-iterators.md) розглядає їх.)
`slices.Sorted` збирає ітератор у новий відсортований зріз, що
згортає весь рецепт відсортованого обходу вище в один рядок:
