...
Коротко попереду (методи розглядаються в окремій темі). Методи прикріплюються через *отримувач* (receiver):

```go
type Celsius float64

// Метод із отримувачем за значенням — працює з копією.
...
Два правила, які варто знати вже зараз:

1. Тип отримувача має бути **визначеним типом** (або вказівником на нього). Не можна написати `func (f float64) ...` — спочатку потрібно визначити `type MyFloat float64`.
2. Тип отримувача має бути **визначений у тому самому пакеті, що й метод**. Не можна додати метод до `time.Duration` з власного пакету. Обхідний шлях: визначте власний тип із `time.Duration` як базовим типом, прикріпіть методи до нього, перетворюйте при перетині меж.
