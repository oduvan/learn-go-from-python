...
Як це запустити — два джерела, злиті в один потік. Функція `gen`, визначена
в розділі «Конвеєр» нижче, надсилає свої аргументи в новий канал. Результати
надходять у будь-якому порядку, тож сортуємо перед друком для сталого
результату:
