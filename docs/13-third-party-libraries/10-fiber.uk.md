...
- **Значення запиту й відповіді повторно використовуються між запитами.**
  ...
  Скопіюйте: `string(append([]byte(nil), b...))`. Цей `append` на
  `nil`-зріз будує новий масив, який нічого не поділяє з буфером
  fasthttp. Або просто використайте рядок від `c.Params`, якщо ваша
  версія вже копіює. Це найважче знайти помилку, бо вона проявляється
  лише під конкурентністю.
